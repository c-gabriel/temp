...
# Joining with external data (this may be removed from the script once the data environment is set)
def data_step1_join_externaldata(df_input):
    df_pd = get_sql_data(''' SELECT * FROM [DIGITAL_PH_DEV].[car_uy].[clasificacion_productos]''')
    df_ba = get_sql_data(''' SELECT * FROM [DIGITAL_PH_DEV].[car_uy].[clasificacion_bu]''')
    df = df_input.copy()
    df = df.astype({'isic':'int64'})
    ...
    df = df[(df['GPH_Group_0_name'] == 'PG&P') & (df['Condition_desc'] == 'Sales of Goods') &
            (~df['nombre_clase_cliente'].isin(['GOVERMENT CUSTOMERS','OTHER COMPANIES','CLIENTE NO CATEGORIZADO'])) & 
            (df['sub_business_area'] != 'HC')]
...
def data_step2a_process_idperiod(df_input, id_column, product_column, time_column, suc_column,
                                 sales_column, mark_column, volume_column, cost_column):
    ...
    df_cost = df.copy()
    df_cost['costo'] = df_cost['costo'].fillna(0)
    df_cost['costo_total'] = df_cost['cantidad_producto'] * df_cost['costo']
    df_cost = df_cost.groupby([id_column,time_column, product_column, suc_column]).agg({'Total_$': 'sum', 'costo_total':'sum'}).reset_index()
    df_cost['margin'] = (df_cost['Total_$'] - df_cost['costo_total']) / df_cost['Total_$']
    df_cost = df_cost[[id_column,time_column, product_column, suc_column,'costo_total','margin']]
    ...
    df2 = df2.merge(df_cost,how='left',on=[id_column,time_column, product_column, suc_column])
...
# Creation of the target variable for cross validation
...
def calibrate_model(df_input, save = False):
 ...
 'Prod_Family_level3', 'Purchase_Ref_Old', 'Ref_Old', 'Market_Segment', 'sucursal'])                            
    y = data['target']
...
def model_and_predict(df_input, rf_calibrated, load=False):
 ...
 'Prod_Family_level3', 'Purchase_Ref_Old', 'Ref_Old', 'Market_Segment', 'sucursal'])   
    ...
    # Load Model
    if load:
        Pkl_Filename = r'C:\Users\c1bo91\OneDrive - Linde Group\Proyectos\001 - Customer At Risk\CAR%20-%20PHS\CAR_UY_Model.pkl' 
...
def risk_classification_gen(df_input):
    ...
    conditions  = [(df_result['frequency'] <= 3),
                   (df_result['Churn_Probability'] >= 0.8),
                   (df_result['Churn_Probability'] >= 0.7) & (df_result['Churn_Probability'] < 0.8),
                   (df_result['Churn_Probability'] >= 0.5) & (df_result['Churn_Probability'] < 0.7),
...
query = '''
SELECT  * 
  FROM [DIGITAL_PH_DEV].[car_uy].[uy_car_data]
...
df_mrg = get_sql_data(''' SELECT * FROM [DIGITAL_PH_DEV].[car_uy].[margenes]''')
df_mrg['Codigo Fo'] = df_mrg['Codigo Fo'].astype('str')
df_mrg['costo'] = df_mrg['Costo unitario'].combine_first(df_mrg['Costo producción'])
df_mrg = df_mrg[['Codigo Fo', 'costo']].rename(columns={'Codigo Fo':'producto'})
...
df_ba = df[['cliente','Market_Segment']].groupby(['cliente']).first().reset_index()
df_v = df[['cliente', 'sucursal', 'Fecha_Emision_ts', 'vendedor',
           'nombre_vendedor']].drop_duplicates().sort_values(by='Fecha_Emision_ts', ascending=False)
df_v = df_v.groupby(['cliente','sucursal']).first().drop(columns=['Fecha_Emision_ts']).reset_index()
df = df.merge(df_mrg, how='left', on='producto')
print('Step 1 OK. Dataset length:', len(df))

# Step 2
df_f1 = data_step2a_process_idperiod(df, 'cliente', 'Prod_Family_level3', 'periodo', 'sucursal', 'Total_$', 'Fecha_Emision', 
                                     'cantidad_producto', 'costo')
df_f1['margin'] = df_f1['margin'].fillna(0)
...
config={'monetary_value_USD': 'mean','monetary_value': 'mean','frequency':'sum','volume': 'mean', 'periodo':'max',
        'max_time_between_tr':'max','margin':'mean'}
...
t_df_past_tr_24M = data_step2b_last_transaction(df_f1, 12, t_attribute_end, 12, ['cliente','Prod_Family_level3', 'sucursal'], 'max').rename(columns={'Purchase_Ref_Old':'Purchase_Ref_Old_24M'})
t_df_past_tr_12M = data_step2b_last_transaction(df_f1, 0, t_attribute_end, 11, ['cliente','Prod_Family_level3', 'sucursal'] ,'min').rename(columns={'Purchase_Ref_Old':'Purchase_Ref_Old_12M'})
t_df_past_tr = df_f1[['cliente','Prod_Family_level3', 'sucursal']].drop_duplicates()
t_df_past_tr = t_df_past_tr.merge(t_df_past_tr_24M, how='left', left_on = ['cliente','Prod_Family_level3', 'sucursal'], right_index=True).merge(t_df_past_tr_12M, how='left', left_on = ['cliente','Prod_Family_level3', 'sucursal'], right_index=True)
...
t_df_f2 = data_step3(df_f1, config, t_attribute_end, 12, 'cliente', 'periodo', 'Prod_Family_level3', 'sucursal', t_df_past_tr)
...
df_past_tr_24M = data_step2b_last_transaction(df_f1, 12, df_f1['periodo'].max(), 12, ['cliente','Prod_Family_level3', 'sucursal'], 'max').rename(columns={'Purchase_Ref_Old':'Purchase_Ref_Old_24M'})
df_past_tr_12M = data_step2b_last_transaction(df_f1, 0, df_f1['periodo'].max(), 11, ['cliente','Prod_Family_level3', 'sucursal'] ,'min').rename(columns={'Purchase_Ref_Old':'Purchase_Ref_Old_12M'})
df_past_tr = df_f1[['cliente','Prod_Family_level3', 'sucursal']].drop_duplicates()
df_past_tr = df_past_tr.merge(df_past_tr_24M, how='left', left_on = ['cliente','Prod_Family_level3' ,'sucursal'], right_index=True).merge(df_past_tr_12M, how='left', left_on = ['cliente','Prod_Family_level3', 'sucursal'], right_index=True)
...
df_f2 = data_step3(df_f1, config, df_f1['periodo'].max(), 12, 'cliente', 'periodo', 'Prod_Family_level3', 'sucursal', df_past_tr)
df_f2 = df_f2.merge(df_cc, how='left', on=['cliente'])
print('Step 3 OK. Dataset length:', len(df_f2))

# Step 4
# Calibration data
t_df_f2_cf = data_step4a(t_df_f2,True,0.999,True,0.01)
t_df_f2_cf['margin'] = np.maximum(t_df_f2_cf['margin'],  1.00) #Ajustar Margen por ahora
t_df_f2_cf = t_df_f2_cf.replace([np.inf, -np.inf], np.nan) #Ajustar Margen por ahora
t_df_f2_cf['margin'] = t_df_f2_cf['margin'].fillna(0) #Ajustar Margen por ahora
...
# Production data
df_f2_cf = data_step4a(df_f2,True,0.999,True,0.01)
df_f2_cf['margin'] = np.maximum(df_f2_cf['margin'],  1.00) #Ajustar Margen por ahora
df_f2_cf = df_f2_cf.replace([np.inf, -np.inf], np.nan) #Ajustar Margen por ahora
df_f2_cf['margin'] = df_f2_cf['margin'].fillna(0) #Ajustar Margen por ahora
...
df_f5 = df_f4.merge(df_m2, how='left', left_on='cliente', right_index=True)
df_f5 = df_f5[['Nombre_cliente','cliente','sucursal','Prod_Family_level3','Churn_Probability', 'periodo_analisis', 'last_purchase',
               'monetary_value','frequency','volume','Mean_time_between_tr','max_time_between_tr','margin']]
...
#Step 6
df_c = df_m3[['Nombre_cliente','cliente']].drop_duplicates()
df_c['Estado'] = ''
df_c['Fecha_planificada'] = ''
df_m4 = df[['cliente','sucursal','telefono_legal','cuin', 'E_mail_legal']].drop_duplicates()
df_m4 = df_m4.merge(df_ba, how='left',on='cliente')
df_c = df_c.merge(df_m4, how='left', on=['cliente'])
df_c = df_c.merge(df_total_sales, how='left', on=['cliente','sucursal'])

df_m3_rk = df_m3[df_m3['Risk_Level'] != 'Oportunidad']
...
df_c_op = df_m3_op.merge(df_m3_op.groupby('cliente').agg({'total_monetary_value':'sum'}).rename(
 columns={'total_monetary_value':'potential_win_opportunity'}), how='left',
            left_on='cliente',right_index=True)
...
print('Clientes en riesgo con oportunidades:', len(df_c[(df_c['client_churn_probability'] >= 0.5) & (~df_c['potential_win_opportunity'].isna())]))
...
df_aux_op = df_aux_op.sort_values(by='total_monetary_value', ascending=False).head(500)
...
df_c2 = df_c2.merge(df_v, how='left', on=['cliente','sucursal']).rename(columns={'nombre_vendedor':'nombre_operario_retencion'})
...
df_c2_old = get_sql_data(''' SELECT * FROM [DIGITAL_PH_DEV].[car_uy].[last_month_clientlist]''')
...
df_c2_old['cliente'] = df_c2_old['cliente'].astype('str')
...
df_tableau_rk =  df_f5.merge(df[['cliente','GPH_Group_0_name', 'sucursal', 'sucursal_emitente','sucursal_cliente', 'telefono_legal' , 'Codigo_postal', 'localidad_legal',
...
df_tableau_rk = df_tableau_rk.merge(df_v, how='left', on=['cliente','sucursal'])
# Exclude one minimal classification -> Mezclas Especiales : Gases Medicinales (less than 10 records) to avoid duplication
df_tableau_rk = df_tableau_rk.merge(df_pd2.drop(21), how='left', on=['Prod_Family_level3']).merge(df_pd3, how='left', on=['Prod_Family_level3'])
...
insert_data_to_sql(df_c3, 'lista_clientes', 'car_uy')
insert_data_to_sql(df_r, 'prod_riesgo', 'car_uy')
insert_data_to_sql(df_tableau_rk, 'car_Tableau_risk_view', 'car_uy')
insert_data_to_sql(df_tableau_hist, 'car_Tableau_historic_view', 'car_uy')
