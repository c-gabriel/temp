...
def data_step2a_process_idperiod(df_input, id_column, product_column, time_column, suc_column,
    ...
    df_maxtbp['max_time_between_tr'] = (df_maxtbp['base_periodo'].dt.year - df_maxtbp['prev_periodo'].dt.year)*12+                                  ((df_maxtbp['base_periodo'].dt.month - df_maxtbp['prev_periodo'].dt.month))
...
def calibrate_model(df_input, save = False):
    ...
    x = data.drop(columns=['CODIGO', 'AGENCIA', 'TIPOPRODSP2', 'Purchase_Ref_Old', 'Ref_Old',
                           'Mean_time_between_tr', 'periodo_analisis', 'SEGMENTO','target'])                            
    y = data['target']
...
def model_and_predict(df_input, rf_calibrated, load=False):
    ...
    x = data.drop(columns=['CODIGO', 'AGENCIA', 'TIPOPRODSP2', 'Purchase_Ref_Old', 'Ref_Old',
                           'Mean_time_between_tr', 'periodo_analisis', 'SEGMENTO']) 
...
query = '''
SELECT *
  FROM [DIGITAL_PH_DEV].[car_ar].[ar_car_data]
  WHERE CONVERT(nvarchar(6), FECHA_COMP, 112) >= {}
...
df_raw = process_sales(
    df_raw,
    '''SELECT * FROM car_ar.car_ventas_master_gerencias''',
    '''SELECT * FROM car_ar.car_ventas_master_tablatipo''',
    '''SELECT * FROM car_ar.ar_productos''',
    '''SELECT * FROM car_ar.ar_clientes''')


# In[4]:


df = df_raw[(~df_raw['TIPOPRODSP2'].isna()) & (df_raw['TABLA_TIPO'].isin(
    ['PRODUCTO GASEOSO','PRODUCTO CO2', 'PRODUCTO OTROS', 'PRODUCTO GE']))]
df['SGRUPO_CLI'] = df['SGRUPO_CLI'].str.strip() 
df = df[~(df['SGRUPO_CLI'].isin(['004-MUNICIPALIDADES/GOBERNACIONES', '010-PROVINCIALES Y MUNICIPALES',
                                 '007-HOMECARE ESTATAL', '002-HOSPITALES PUBLICOS', '002-ORGANISMOS PUBLICOS']))]
df = df[~(df['SEGMENTO'].isin(['COMPETIDORES']))]
print('Step 1 OK. Dataset length:', len(df))

df_f1 = data_step2a_process_idperiod(df, 'CODIGO', 'TIPOPRODSP2', 'periodo', 'AGENCIA', 'PESOS', 'FECHA_COMP', 
                                     'VOLUMEN')
...
client_column = 'CODIGO'
product_column = 'TIPOPRODSP2'
sucursal_column = 'AGENCIA'
recover = True
...
# Step 4
#Dummies de ARG
df_add = df_raw[['CODIGO','TIPOPRODSP2','SEGMENTO','CLIENTE_MAS','CORPORATIVO',
                 'BARRACAS', 'DISTRIBUIDORES']].drop_duplicates()

df_add['CLIENTE_MAS'] = np.where(df_add['CLIENTE_MAS'] == 'S', 1, 0)
df_add['CORPORATIVO'] = np.where(df_add['CORPORATIVO'] == 'S', 1, 0)
df_add['BARRACAS'] = np.where(df_add['BARRACAS'] == 'S', 1, 0)
df_add['DISTRIBUIDORES'] = np.where(df_add['DISTRIBUIDORES'] == 'S', 1, 0)
...
df_f3_cj = data_step4b(df_f3_cj, ['AGENCIA', 'TIPOPRODSP2', 'SEGMENTO'])
...
df_m2 = df_raw[[client_column,'CLIENTE']].drop_duplicates().groupby(client_column).CLIENTE.first()
...
df_f5 = df_f5[['CLIENTE',client_column,sucursal_column,product_column,'Churn_Probability',
...
#Step 6
df_c = df_m3[['CLIENTE',client_column]].drop_duplicates()
df_c['Estado'] = ''
df_c['Fecha_planificada'] = ''
df_m4 = df_raw[[client_column,sucursal_column,'SEGMENTO','CC','FACTURACION','CLIENTE_MAS','CORPORATIVO',
                'BARRACAS','DISTRIBUIDORES']].drop_duplicates()
df_vendedor_m4 = df_raw[[client_column, 'VENDEDOR', 'FECHA_COMP']].sort_values(by='FECHA_COMP', ascending=False)
df_vendedor_m4 = df_vendedor_m4.groupby(['CODIGO']).first().reset_index()
df_c = df_c.merge(df_m4, how='left', on=[client_column])
df_c = df_c.merge(df_vendedor_m4, how='left', on=[client_column])
df_c = df_c.merge(df_total_sales.rename(columns={'PESOS':'Facturacion12M'})
...
df_contacto = df[['CODIGO','AGENCIA', 'NRODOC_CLI', 'TELEFO_CLI','EMAIL1_CLI']].drop_duplicates()
df_c = df_c.merge(df_contacto, how='left',on=['CODIGO','AGENCIA'])
...
df_aux_op = df_m3[(df_m3['Risk_Level'] == 'Oportunidad') & 
                  ...
                  (pd.to_datetime(df_m3['last_purchase'], format='%Y%m') >= (pd.to_datetime(df['periodo'].max(), format='%Y%m') + relativedelta(months=-4)))][['CLIENTE',client_column,'total_monetary_value','margin']].drop_duplicates()
...
df_aux_rk = df_m3[(~df_m3['Risk_Level'].isin(['Oportunidad','Compra Rcte'])) &
                  (df_m3['Churn_Probability'] >= 0.5)][['CLIENTE',
...
df_r = (df_f5[(df_f5[client_column].isin(df_c2[client_column].unique())) &
    (((df_f5['Churn_Probability'] >= 0.5) & (df_f5['Risk_Level'] != 'Compra Rcte')) #NEW
               | (df_f5['Risk_Level'] == 'Oportunidad'))]).drop(columns=['CLIENTE'])
...
if recover:
    df_c2_old = get_sql_data(''' SELECT * FROM [DIGITAL_PH_DEV].[car_ar].[last_month_clientlist]''')
...
df_tableau_rk =  df_f5.merge(df[[client_column, product_column, sucursal_column, 'GERENCIA_NACIONAL', 'GERENCIA_REGIONAL',
                                 'DOMICI_CLI', 'LOCALI_CLI', 'CODPOS_CLI', 'PROVIN_CLI', 'GRUPO__CLI',
                                 'SGRUPO_CLI', 'FORMADIST', 'TABLA_TIPO', 'VENDEDOR']].drop_duplicates(),
                             how='left', on=[client_column, product_column, sucursal_column])
df_tableau_rk = df_tableau_rk.merge(df_total_sales, how='left', on=[client_column,sucursal_column])
df_tableau_rk = df_tableau_rk.merge(df_add, how='left', on=[client_column,product_column])
...
df_tableau_rk = df_tableau_rk.rename(columns = {'PESOS':'Facturacion12M'})
...
insert_data_to_sql(df_c3, 'lista_clientes', 'car_ar')
insert_data_to_sql(df_r, 'prod_riesgo', 'car_ar')
insert_data_to_sql(df_tableau_rk, 'car_Tableau_risk_view', 'car_ar')
insert_data_to_sql(df_tableau_hist, 'car_Tableau_historic_view', 'car_ar')

print('Step 7 OK. Data exported.')
...
df_c2_powerapp = categorize_risk(df_c2, df_r, client_column, sucursal_column)
export_to_excel(r'C:\Users\c1bo91\OneDrive - Linde Group\Proyectos\001 - Customer At Risk\Argentina\PowerApp_Files\car_ar_clientes.xlsx',
        'AR_CAR_clients', df_c2_powerapp)


# In[79]:


df_c2_powerapp['Risk_Level_n'].unique()
...
pd.DataFrame(df_c2.groupby('VENDEDOR').CODIGO.nunique().sort_values(ascending=False))


# In[78]:


pd.DataFrame(df_c.groupby('VENDEDOR').CODIGO.nunique().sort_values(ascending=False))


# In[20]:


df_1 = df_raw[(~df_raw['TIPOPRODSP2'].isna()) & (df_raw['TABLA_TIPO'].isin(
    ['PRODUCTO GASEOSO','PRODUCTO CO2', 'PRODUCTO OTROS', 'PRODUCTO GE']))]
df_1['SGRUPO_CLI'] = df_1['SGRUPO_CLI'].str.strip() 
df_1 = df_1[~(df_1['SEGMENTO'].isin(['COMPETIDORES']))]
print(len(df_1))

df_2 = df_1[~(df_1['SGRUPO_CLI'].isin(['004-MUNICIPALIDADES/GOBERNACIONES', '010-PROVINCIALES Y MUNICIPALES',
                                             '007-HOMECARE ESTATAL']))]
print(len(df_2))
...
pd.DataFrame((df_1.groupby('SGRUPO_CLI').size()/len(df_1)).sort_values(ascending=False)).head(50)
...
print('Total de clientes:', len(df_c))
print('Clientes en riesgo:', (df_r[df_r['Risk_Level'].isin(['Perdido','En Riesgo','En Riesgo Alto'])]['CODIGO'].nunique()))


# In[98]:


df_tableau_rk[df_tableau_rk['VENDEDOR'].isna()]


# In[25]:


df_f5['CODIGO'].nunique()


# In[96]:


pd.DataFrame(df_tableau_rk.groupby('VENDEDOR').CODIGO.nunique().sort_values(ascending=False))


# In[55]:


df_tableau_rk.columns


# In[107]:


df_tableau_rk['VENDEDOR'].unique()
...
m = df_tableau_rk[(df_tableau_rk['VENDEDOR'].isin(['GABRIEL ALBERTO ALARCON       ',
                                                   'CIBAU FERNANDO                ',
                                                   'MARCOS DELFOR FERNANDEZ       ',
                                                   'JOAQUIN BASTIDA               '])) &
              (df_tableau_rk['Risk_Level'].isin(['Perdido','En Riesgo','En Riesgo Alto']))]['CODIGO'].unique()

df_tableau_rk[df_tableau_rk['CODIGO'].isin(m)].to_excel('tempora_car_arg.xlsx')
