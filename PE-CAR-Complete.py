...
def process_sales(data_input):
    ...
    data_out['periodo'] = data_out['venta_tiempo_id'].astype(str).str.slice(start=0, stop=6)
    data_out['tipo_id'] = np.where(data_out['cliente_ruc_dni'].astype(str).str[:3] == 'DNI', 'DNI', 'RUC')
    return data_out
...
def calculate_total_sale_last_12M(df_input):
    ...
    df_analysis = df_analysis.groupby(['CodigoClienteFull','sucursal_descripcion']).agg({'Venta en S/':'sum'}).reset_index()
    return df_analysis
...
def calibrate_model(df_input, save = False):
    ...
    x = data.drop(columns=['CodigoClienteFull', 'sucursal_descripcion', 'Producto_SPGrupo_descripcion','target',
                          'Purchase_Ref_Old','Ref_Old','periodo_analisis'])                            
    y = data['target']
...
def model_and_predict(df_input, rf_calibrated, load=False):
    ...
    x = data.drop(columns=['CodigoClienteFull', 'sucursal_descripcion', 'Producto_SPGrupo_descripcion',
                          'Purchase_Ref_Old','Ref_Old','periodo_analisis']) 
...
query_contact = '''
SELECT *
  FROM [DIGITAL_PH_DEV].[stage].[pe_car_contactdata]
  '''
df_tel = get_sql_data(query_contact)
df_tel = df_tel.drop_duplicates()
df_tel['CodigoClienteFull'] = df_tel['CodigoClienteFull'].astype('int64')
df_tel = df_tel.rename(columns={'Compa__a':'Compañía', 'Codigo_Sucursal':'Codigo Sucursal',
                                'Codigo_Cliente': 'Codigo Cliente'})

reference_time = pd.to_datetime('today') + relativedelta(months=-27)
query = '''
SELECT *
  FROM [DIGITAL_PH_DEV].[car_pe].[pe_car_data]
  WHERE venta_tiempo_id > {}
'''.format(str(reference_time.year)+str(reference_time.month)+'01')

df_raw = get_sql_data(query)
df_raw = process_sales(df_raw)

df_raw['fecha_comp'] = pd.to_datetime(df_raw['venta_tiempo_id'], format = "%Y%m%d")


df = df_raw[(df_raw['producto_packaged'] == 1) & (df_raw['venta_servicio'] != 1) & (df_raw['venta_flete'] != 1)]
df = df[(~df['Producto'].isin(['Oxigeno Medicinal 99.5% V/v Liquido Criogénico','Nitrogeno Liquido',
                              'Oxigeno Liquido Industrial','Oxido Nitroso 99.0% V/v Gas Licuado']))]
df = df[(df['Cliente_Aplicacion'] != 'Home Care/ Atencion Domicilio')]

df_new_class = get_sql_data('''SELECT * FROM [DIGITAL_PH_DEV].[car_pe].[mapping_ge_prods]''').drop_duplicates()

df = df.merge(df_new_class.drop(columns=['Producto_SPGrupo_descripcion']), how='left', on=['Producto'])
df['Producto_SPGrupo_descripcion'] = np.where(df['Producto_SPGrupo_descripcion_new'].isna(),
                                              df['Producto_SPGrupo_descripcion'],
                                              df['Producto_SPGrupo_descripcion_new'])
df = df.drop(columns=['Producto_SPGrupo_descripcion_new'])
df = df[df['tipo_id']=='RUC']

df_f1 = data_step2a_process_idperiod(df, 'CodigoClienteFull', 'Producto_SPGrupo_descripcion', 'periodo', 'sucursal_descripcion',
                                     'Venta en S/', 'fecha_comp', 'Venta en Volumen')

# Step 3
# Calibration set up
config={'monetary_value': 'mean','frequency':'sum','volume': 'mean', 'periodo':'max',
        'max_time_between_tr':'max'}
t_attribute_end = pd.to_datetime(df['periodo'].max(), format='%Y%m') + relativedelta(months=-3)
t_attribute_end = str(t_attribute_end.year) + str(t_attribute_end.month)
df_total_sales = calculate_total_sale_last_12M(df)

client_column = 'CodigoClienteFull'
product_column = 'Producto_SPGrupo_descripcion'
sucursal_column = 'sucursal_descripcion'
recover = True
...
if len(t_df_past_tr_24M) != 0:
    t_df_past_tr = t_df_past_tr.merge(t_df_past_tr_24M, how='left', left_on = [client_column,product_column, sucursal_column], right_index=True)
else:
    t_df_past_tr['Purchase_Ref_Old_24M'] = np.nan
t_df_past_tr = t_df_past_tr.merge(t_df_past_tr_12M, how='left', left_on = [client_column,product_column, sucursal_column], right_index=True)
...
if len(df_past_tr_24M) != 0:
    df_past_tr = df_past_tr.merge(df_past_tr_24M, how='left', left_on = [client_column, product_column, sucursal_column], right_index=True)
else:
    df_past_tr['Purchase_Ref_Old_24M'] = np.nan
df_past_tr = df_past_tr.merge(df_past_tr_12M, how='left', left_on = [client_column,product_column, sucursal_column], right_index=True)
...
t_df_f3 = t_df_f2_cf.copy()
# t_df_f3 = t_df_f2_cf.merge(df_add, how='inner', on=[client_column, product_column])
...
df_f3 = df_f2_cf.copy()
# df_f3 = df_f2_cf.merge(df_add, how='inner', on=[client_column, product_column])
df_f3['data_type'] = 'Production'

df_f3_cj = pd.concat([t_df_f3, df_f3], ignore_index=True)
df_f3_cj = data_step4b(df_f3_cj,['sucursal_descripcion', 'Producto_SPGrupo_descripcion'])
...
t_df_f3_2[client_column] = t_df_f3_2[client_column]
t_df_target['CodigoClienteFull'] = t_df_target['CodigoClienteFull'].astype('int64')
...
df_m2 = df_raw[[client_column,'cliente_descripcion']].drop_duplicates().groupby(client_column).cliente_descripcion.first()
# df_f5 = df_f4.merge(df_m1, how='left', on=[client_column,product_column])
df_f4[client_column] = df_f4[client_column].astype('int64')
df_m2 = df_m2.reset_index()
df_m2[client_column] = df_m2[client_column].astype('int64')
df_f5 = df_f4.merge(df_m2, how='left', on=client_column)
df_f5 = df_f5[['cliente_descripcion',client_column,sucursal_column,product_column,'Churn_Probability',
...
#Step 6
df_c = df_m3[['cliente_descripcion',client_column]].drop_duplicates()
df_c['Estado'] = ''
df_c['Fecha_planificada'] = ''
df_m4 = df_raw[[client_column,sucursal_column,'cliente_ruc_dni','clt_vendedor_descripcion',
                'Cliente_Industria' ,'Cliente_Aplicacion', 'fecha_comp']].drop_duplicates()
df_m4 = df_m4.sort_values(by='fecha_comp', ascending=False)
df_m4 = df_m4.groupby([client_column,sucursal_column]).first().reset_index()
df_m4[client_column] = df_m4[client_column].astype('int64')
df_c[client_column] = df_c[client_column].astype('int64')
df_c = df_c.merge(df_m4, how='left', on=[client_column])
df_total_sales[client_column] = df_total_sales[client_column].astype('int64')
df_c = df_c.merge(df_total_sales.rename(columns={'Venta en S/':'Facturacion12M'})
...
df_aux_op = df_m3[(df_m3['Risk_Level'] == 'Oportunidad') & 
                  ...
                  (pd.to_datetime(df_m3['last_purchase'], format='%Y%m') >= (pd.to_datetime(df['periodo'].max(), format='%Y%m') + relativedelta(months=-4)))][['cliente_descripcion',client_column,'total_monetary_value','margin']].drop_duplicates()
...
df_aux_rk = df_m3[(~df_m3['Risk_Level'].isin(['Oportunidad','Compra Rcte'])) &
                  (df_m3['Churn_Probability'] >= 0.5)][['cliente_descripcion',
...
df_r = (df_f5[(df_f5[client_column].isin(df_c2[client_column].unique())) &
    (((df_f5['Churn_Probability'] >= 0.5) & (df_f5['Risk_Level'] != 'Compra Rcte')) #NEW
               | (df_f5['Risk_Level'] == 'Oportunidad'))]).drop(columns=['cliente_descripcion'])
df_c2 = df_c2[df_c2[client_column].isin(df_r[client_column].unique())]

# CCC assignment
# alloc_list = {'CCC - Luz Castañeda':0.3,'CCC - Andres Torres':0.7}
# df_c2 = customer_center_allocation(df_c2, alloc_list)  
...
if recover:
    df_c2_old = get_sql_data(''' SELECT * FROM [DIGITAL_PH_DEV].[car_pe].[last_month_clientlist]''')
...
print('Step 7 OK. Process ended.')

df_master = df.sort_values(by=[client_column,
            'venta_tiempo_id'], ascending=False).groupby([client_column, sucursal_column]).first().reset_index()
df_master = df_master[[client_column, sucursal_column,'clt_vendedor_descripcion',
                   'Ubigeo_Legal','Ubigeo_Entrega','Cliente_Industria']].drop_duplicates()

df_masterapp = df.sort_values(by=[client_column,
            'venta_tiempo_id'], ascending=False).groupby([client_column, sucursal_column,
                                                          product_column]).first().reset_index()
df_masterapp = df_masterapp[[client_column, sucursal_column, product_column, 'Cliente_Aplicacion']].drop_duplicates()

df_f5[client_column] = df_f5[client_column].astype('int64')
df_master[client_column] = df_master[client_column].astype('int64')
df_tableau_rk =  df_f5.merge(df_master, how='left', on=[client_column, sucursal_column])
df_masterapp[client_column] = df_masterapp[client_column].astype('int64')
df_tableau_rk =  df_tableau_rk.merge(df_masterapp, how='left', on=[client_column, product_column, sucursal_column])
...
df_tableau_rk = df_tableau_rk.rename(columns={'Venta en S/':'Facturacion12M'})
df_tableau_hist = df_raw[(df_raw['producto_packaged'] == 1) &
                         (df_raw['venta_servicio'] != 1) &
                         (df_raw['venta_flete'] != 1)].copy()

month = ((pd.to_datetime("today")+relativedelta(months=-1)).month)
year = ((pd.to_datetime("today")+relativedelta(months=-1)).year)
df_tableau_hist = df_tableau_hist[((pd.to_datetime(df_tableau_hist['fecha_comp'], format='%Y%m%d')).dt.year == year) &
                ((pd.to_datetime(df_tableau_hist['fecha_comp'], format='%Y%m%d')).dt.month == month)]
df_c3 = df_c2.copy()
...
insert_data_to_sql(df_c3, 'lista_clientes', 'car_pe', mode='replace')
insert_data_to_sql(df_r, 'prod_riesgo', 'car_pe', mode='replace')
insert_data_to_sql(df_tableau_rk, 'car_Tableau_risk_view', 'car_pe')
insert_data_to_sql(df_tableau_hist.drop(columns=['tipo_id']), 'car_Tableau_historic_view', 'car_pe')
