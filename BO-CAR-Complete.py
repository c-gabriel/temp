...
def calculate_total_sale_last_12M(df_input):
    ...
    df_analysis = df_analysis[(df_analysis['FECHA'] >= time_min)]
    df_analysis = df_analysis.groupby(['NO_CLIENTE','CENTROD']).agg({'TOTAL':'sum'}).reset_index()
    return df_analysis
...
#     encoded = encoded.drop(columns=['CENTROD','TIPO_CLIENTE'])
...
def calibrate_model(df_input, save = False):
    ...
    x = data.drop(columns=['SEGMENTO','NO_CLIENTE','CENTROD','GRUPO_NSP','Purchase_Ref_Old','Ref_Old',
                        'periodo_analisis','TIPO_CLIENTE','target'])                            
    y = data['target']
...
query = '''
SELECT *
  FROM [DIGITAL_PH_DEV].[car_bo].[bo_car_data]
...
df = df_raw[(df_raw['GRUPO_ICC'].isin(['PACKAGED MEDICINAL', 'PACKAGED INDUSTRIAL', 'CO2', np.nan])) &
                 ~(df_raw['GRUPO_NSP'].isin(['Liquid Helium Medical','Liquid CO2'])) &
                 ~(df_raw['GRUPO'].isin([2, 3])) & (df_raw['TIPO_CLIENTE'] == 'PV') &
                 ~(df_raw['SEGMENTO'].isin(['HOME CARE', 'HOMECARE']))].drop_duplicates()

df_f1 = data_step2a_process_idperiod(df, 'NO_CLIENTE', 'GRUPO_NSP', 'periodo', 'CENTROD', 'TOTAL',
                                     'FECHA', 'VOLUMEN')
...
product_column = 'GRUPO_NSP'
sucursal_column = 'CENTROD'
recover = True
...
# Step 4
#Dummies de PAR
df_add = df_raw[['NO_CLIENTE','CENTROD','GRUPO_NSP','TIPO_CLIENTE',
                 'SEGMENTO']].drop_duplicates()
...
df_f3_cj = data_step4b(df_f3_cj_pre, [sucursal_column, product_column, 'SEGMENTO'])
...
df_m4 = df_raw[[client_column,sucursal_column,'TIPO_CLIENTE','SEGMENTO']].drop_duplicates()
df_c = df_c.merge(df_m4, how='left', on=[client_column])
df_c = df_c.merge(df_total_sales.rename(columns={'TOTAL':'Facturacion12M'})
...
print('C1:',len(df_c))
df_c = df_c.merge(df_raw[['NO_CLIENTE','CENTROD','TELEFONO','EMAIL1','ASESOR_COMERCIAL']].drop_duplicates(),
                  how='left',on=['NO_CLIENTE','CENTROD'])
print('C2:',len(df_c))
...
if recover:
    df_c2_old = get_sql_data(''' SELECT * FROM [DIGITAL_PH_DEV].[car_bo].[last_month_clientlist]''')
...
master_data = df.sort_values(by='periodo', ascending=False)[[client_column, product_column, sucursal_column,
        'GRUPO', 'TIPO_CLIENTE', 'DIRECCION','ZONA','PAIS','PROVINCIA','CANTON','DISTRITO','BARRIO',
                 'DESCRIPCION_1','ASESOR_COMERCIAL', 'SEGMENTO']].drop_duplicates().groupby([client_column,
...
df_tableau_rk = df_tableau_rk.rename(columns = {'TOTAL':'Facturacion12M'})
...
df_tableau_hist = df_tableau_hist[(df_tableau_hist['FECHA'].dt.year == year) &
                (df_tableau_hist['FECHA'].dt.month == month)]
df_c3 = df_c2.copy()
...
insert_data_to_sql(df_c3, 'lista_clientes', 'car_bo')
insert_data_to_sql(df_r, 'prod_riesgo', 'car_bo')
insert_data_to_sql(df_tableau_rk, 'car_Tableau_risk_view', 'car_bo')
insert_data_to_sql(df_tableau_hist, 'car_Tableau_historic_view', 'car_bo')
